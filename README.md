...
### 🤖 角色扮演对话
- **380+ 位罗德岛干员**：20 位手写干员拥有详尽人格设定（身份背景、核心性格、语气特征、知识领域、经典台词），另有 364 位全量干员档案（自动生成人格 + PRTS wiki 立绘），全部基于 LLM 大语言模型驱动
...
### 💬 群聊系统
- **多人干员群聊**：创建 2–10 人群聊频道，发一条消息后群成员会**依次流式回复**（后发言者能看到先发言者的回复，对话自然衔接）
- **@ 提及指定发言人**：输入 `@` 弹出成员选择器（或点击顶部成员头像条），被 @ 的成员必定优先回应
- **发言人选择智能随机**：无 @ 时随机 1–4 位成员接话，有 @ 时被提及者在前、其余随机补齐（移植自安卓版调度算法）
- **群聊独立历史**：每个群独立保存最多 100 条消息，群列表显示最新消息预览；支持删除群
- **群聊不触发自动视频**：与安卓版一致，避免群聊产生额外费用

### 🔍 干员/人员搜索
- **覆盖 400+ 干员**：内置 20 位手写干员 + **364 位全量干员档案**（含 PRTS wiki 立绘直链）+ 自定义角色
- **搜索框实时过滤**：按名字 / 代号子串匹配，桌面右栏与移动端角色页共用
- **建群成员选择器**：同样支持搜索过滤，400+ 干员一目了然

### 👤 我的形象设置
- **博士档案**：设置昵称、人设简介、与干员的关系，自动注入每次对话的 system prompt，让 AI 记住"你是谁"
- **自定义头像**：上传头像（自动压缩），聊天中展示

### 🖼️ 自定义聊天背景
- **多图轮播**：上传最多 20 张背景图，8 秒自动切换（沿用内置轮播的渐变过渡）
- **开关控制**：随时在设置中切换自定义背景 / 内置背景

### 🎬 视频生成（Seedance）
- **对话视频**：把角色回复演绎成 4–15 秒短视频（角色立绘 + 可选背景图作为参考，角色"说出"这句回复）
- **双协议支持**：火山方舟官方 API / 中转站媒体 API 自动识别
- **自动 + 手动开关**：默认关闭（省钱），可开启「每轮回复自动生成」；每条回复旁也有手动生成按钮
- **完整状态机**：分镜生成 → 创建任务 → 轮询 → 下载，失败可手动重试（涉及费用的失败需确认，绝不重复计费）
- **测试连接**：设置面板可一键验证服务地址与密钥，不产生费用

### 🧩 自定义角色导入
- **可视化导入面板**：点击角色区「＋ 导入角色」按钮，打开 PRTS 风格导入弹窗，填写角色名 / 代号 / 种族 / 定位 / 系统提示词，右侧实时预览角色卡效果
- **本地素材管理**：立绘（选择区 + Live2D 区）、点击语音支持「本地上传」或「粘贴 URL」两种方式；上传的二进制素材存入 **IndexedDB**，元数据存入 localStorage，刷新不丢失
- **音色码与字幕**：可为自定义角色填写火山引擎中/日文音色码（`S_xxxx`）与点击触发的双语字幕，TTS 与点击语音自动接入
- **角色卡管理**：自定义角色以「自定义」角标显示在选择区，支持卡片内编辑 ✎ / 删除 ✕，与内置干员一同参与切换、聊天、TTS、移动端下拉
- **JSON 备份与分享**：支持将单个角色导出为 JSON（含 base64 内嵌素材），可导入还原，方便跨设备迁移或分享给他人
...
- **底部标签栏**：「聊天」「群聊」「角色」「音乐」四个 tab 一键切换主视图，常用操作触手可及
- **抽屉式角色/音乐面板**：左右面板改为全屏固定覆盖层 (fixed overlay)，带滑入/滑出透明度动画，不挤占聊天区域
- **设置侧滑抽屉**：聊天顶栏新增齿轮按钮 ⚙️，点击从右侧滑出设置面板（手风琴分区：API / TTS / 我的形象 / 聊天背景 / 视频生成 / 性能），无需离开聊天界面即可调整配置
- **群聊移动端交互**：群聊 tab 查看群列表 → 点击进入群会话（自动切回聊天视图），顶部成员头像条横滑、输入 `@` 弹出成员选择器、返回按钮一键回到单聊
- **键盘适配**：`visualViewport` + `interactive-widget=resizes-content`，键盘弹起时输入区与 @ 选择器自动上移；横屏（landscape）自动压缩输入区与标签栏
- **移动端性能**：默认关闭 Live2D（省电，可在设置「性能」中开启）、聊天背景透明度降噪、SSE 流式渲染节流（150ms）、翻历史时不强制拉底
...
除内置 20 位干员外，可导入任意自定义角色参与对话：

1. 在角色选择区点击「＋ 导入角色」按钮，打开导入弹窗
2. 填写**角色名称**与**系统提示词**（必填），以及代号 / 种族 / 定位等可选信息
3. 上传或粘贴**选择区立绘** / **Live2D 区立绘** / **点击语音**（至少一张立绘以正常显示卡片）
4. 可选：填写火山引擎**中/日文音色码**与点击触发的**双语字幕**
5. 点击「保存角色」，自定义角色即出现在选择区（带「自定义」角标），可随时编辑 ✎ / 删除 ✕
6. 点击「导出 JSON」可备份单个角色（含素材），「导入 JSON」可从备份文件还原
